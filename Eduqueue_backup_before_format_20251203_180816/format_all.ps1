...
Get-ChildItem -Path $workspace -Recurse -File | Where-Object { $_.FullName -notlike "$backupDir*" } | ForEach-Object {
...
foreach ($ext in $exts) {
    Get-ChildItem -Path $workspace -Recurse -Include $ext -File | Where-Object { $_.FullName -notlike "$backupDir*" } | ForEach-Object {
        $path = $_.FullName
        try {
